...
- Files matching `*.test.*` or `*.spec.*` or `test_*.py` or `*_test.go` or `*Test.java` or `*Test.kt` or `*_spec.rb` or `*Test.php` or `*Tests.cs` or `*_test.dart` or `*Tests.m` -> `test`
...
- Files named `Application.java` or `Application.kt` or `Program.cs` -> `entry` (JVM / .NET entry points)
- Files named `main.dart` in `lib/` -> `entry` (Dart/Flutter entry point)
- Files named `AppDelegate.m` or `main.m` -> `entry` (Objective-C entry point)
